...
If a stage exits non-zero, the built-in executor reports an observed failing stage through ``ShellError/exitFailure(command:output:)-enum.case`` and cancels the remaining stage tasks. Because stages are concurrent, simultaneous failures do not have a deterministic pipeline-order winner. A non-final stage terminated by `SIGPIPE` is not treated as a failure, because it only means a downstream stage stopped reading early — for example `yes | head -n 1` succeeds. Timeout, cancellation, and output-limit failures include captured partial output.
