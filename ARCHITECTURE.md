...
- The executor races each run (or pipeline) against its timeout in one structured task group: the loser is cancelled, cancelling the run makes swift-subprocess kill the process group, and leaving the group waits for the processes to be reaped. Exceeding the output limit kills the offending process group from the stream-reading task itself, which unblocks the other stream so the run can unwind. There is no unstructured task or event actor per run.
...
- Commands never inherit the calling process's stdin. A command reads its `InputSource` (`Command.stdin(_:)`): empty by default (reads hit end-of-file), or fixed text, bytes, or a file. Text and bytes reach the child as a file descriptor, never through swift-subprocess `DataInput` (whose failed write, when the child exits without reading everything, traps on an unclosed descriptor and on Linux raises `SIGPIPE` in the caller): on Linux an in-memory `memfd_create` file, on Darwin a pipe that SwiftyShell fills from a dispatch queue with its write end marked `F_SETNOSIGPIPE`. Large inputs cannot deadlock against large outputs, and a child that ignores its input is not an error. A pipeline's first stage reads its own source; later stages receive the preceding stage's stdout, and a source set on a later stage is rejected with `invalidConfiguration` before any stage starts. Writing to a running process's stdin interactively is still deferred.
