...
Use `MockExecutor` for unit tests. It implements `CommandExecutor`, mirrors real `run()` failure semantics for non-zero exits, validates timeout/output-limit configuration, and returns caller-supplied responses without spawning processes. It records every command it receives in `recordedCommands` (use that instead of hand-rolled recorder actors) and can answer from ordered `Stub`s via `init(stubs:fallback:)`, throwing `commandNotFound` for unmatched commands. Its pipeline handling mirrors production: all stages validated then invoked, final-stage stdout plus every stage's stderr, first failing stage in pipeline order. Its `spawn` support returns `MockSpawnedProcess`, which records signals, teardown, and the configured `TeardownStrategy` so spawned-process code can be tested without launching a subprocess. Integration tests that require real executables should be clearly annotated.
