...
public struct Bun: RunnableCommandFamily {
    ...
    public func build(_ entrypoints: [String]) -> Self {
        modified(self) {
            $0.state.subcommand = "build"
            $0.state.scriptName = nil
            $0.state.buildEntrypoints = entrypoints
            $0.state.positionals = []
        }
    }
    ...
    public func hot(_ enabled: Bool = true) -> Self { modified(self) { $0.state.usesHotReload = enabled } }
